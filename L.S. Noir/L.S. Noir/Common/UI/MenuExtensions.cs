...
using static RAGENativeUI.Common;
...
namespace LSNoir.Common.UI
{
    internal static class MenuExtensions
    {
        ...
        /// This should called only once per menu with all the items that need fast scrolling.
        /// </summary>
        public static UIMenu WithFastScrollingOn(this UIMenu menuParam, IEnumerable<UIMenuItem> items)
        {
            ...
            void UpdateAcceleration(UIMenu menu, UIMenuItem selectedItem)
            {
                ...
                menu.SetKeyAcceleration(MenuControls.Left, accel);
                menu.SetKeyAcceleration(MenuControls.Right, accel);
            }

            menuParam.OnIndexChange += (m, i) => UpdateAcceleration(m, m.MenuItems[i]);

            if (menuParam.MenuItems.Count > 0)
            {
                // in case the currently selected item is one of the items that need fast scrolling
                UpdateAcceleration(menuParam, menuParam.MenuItems[menuParam.CurrentSelection]);
            }
        ...
        private static void TrimAndSetRightLabel(UIMenuItem item, string str, int maxLength) => item.RightLabel = str.Length > maxLength ? (str.Substring(0, maxLength) + "...") : str;
        ...
        private static void NotifyIncorrectInput(string msg) => Game.DisplayNotification($"~r~Incorrect input~s~: {msg}");
